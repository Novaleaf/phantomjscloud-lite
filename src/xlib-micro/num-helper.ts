...
export function hash( input: number ): number {
	...
	if ( !( input <= INT32_MAX && input % 1 === 0 ) ) { throw new ex.Exception( "must supply integer" ); }
...
export function parseIntEx( toParse: any,/** invalid strings are returned as this (default=NaN) */invalidResult = NaN, radix = 10 ) {
...
export function parseFloatEx( toParse: any,/** invalid strings are returned as this (default=NaN) */invalidResult = NaN, /** the default parseFloat implementation allows for trailing text.  specifying isStrict=TRUE makes only numbers + "Infinity" allowed to be parsed.   */isStrict = false ) {
