...
import ex = require( "./xlib-micro/exception" );
import net = require( "./xlib-micro/net" );
...
*/
export class PhantomJsCloudException extends ex.Exception { }
/**
...
export interface IBrowserApiOptions {
	...
	*/
    ...
    endpointOptions?: net.IRemoteHttpEndpointOptions;
}
// /**
...
export class BrowserApi {
    ...
    constructor(/**options, or pass your PhantomJsCloud.com ApiKey here.   If you don't, you'll use the "demo" key, which is good for about 100 pages/day.   Signup at https://Dashboard.PhantomJsCloud.com to get 500 Pages/Day free*/  keyOrOptions: string | IBrowserApiOptions = {} as any ) {
        ...
        this._endpoint = new net.RemoteHttpEndpoint<ioDatatypes.IUserRequest, ioDatatypes.IUserResponse>( this.options.endpointOptions );
    }

    /** @hidden the low-level endpoint used to make the requests.  exposed for debugging support only. */
    public _endpoint: net.RemoteHttpEndpoint<ioDatatypes.IUserRequest, ioDatatypes.IUserResponse>;
	...
	*/
    public async requestSingle( request: ioDatatypes.IUserRequest | ioDatatypes.IPageRequest,
        callback?: ( err?: axios.AxiosError, result?: ioDatatypes.IUserResponse ) => void ): Promise<ioDatatypes.IUserResponse> {
        ...
        ///////////////////////////////////////  await inspect mode
        // //try {        
        const { toInspect } = await promise.awaitInspect( this._endpoint.post( userRequest ) );
        if ( toInspect.isFulfilled() ) {
            ...
            const err = toInspect.reason() as axios.AxiosError;
            if ( callback != null ) {
                callback( err, undefined );
            }
            return bb.reject( err );
