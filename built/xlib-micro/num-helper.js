...
function hash(input) {
    ...
    if (!(input <= exports.INT32_MAX && input % 1 === 0)) {
        throw new ex.Exception("must supply integer");
    }
...
function clamp(value, min_inc, max_inc, /** defaults to "clamp" */ clampType = ClampType.clamp) {
    ...
    switch (clampType) {
        ...
        case ClampType.bounce:
            ...
            return max_inc - modRemainder;
        default:
            throw new ex.Exception("unknown ClampType: " + ClampType[clampType]);
    }
}
exports.clamp = clamp;
...
function parseIntEx(toParse, /** invalid strings are returned as this (default=NaN) */ invalidResult = NaN, radix = 10) {
    //logger.assert(typeof (toParse) === "string", "input is not a string");
    let result = parseInt(toParse, radix);
    if (isNaN(result)) {
...
exports.parseIntEx = parseIntEx;
/** parses the value into a float. */
function parseFloatEx(toParse, /** invalid strings are returned as this (default=NaN) */ invalidResult = NaN, /** the default parseFloat implementation allows for trailing text.  specifying isStrict=TRUE makes only numbers + "Infinity" allowed to be parsed.   */ isStrict = false) {
    let result;
    if (isStrict === true) {
        if (/^(\-|\+)?([0-9]+(\.[0-9]+)?|Infinity)$/
            .test(toParse)) {
...
exports.parseFloatEx = parseFloatEx;
        ...
        switch (str) {
            ...
            default:
                if (throwOnInvalid === true) {
                    throw new ex.Exception("unable to parseBoolean on input value= " + toParse);
                }
                return invalidResult;
        }
    }
...
//	});
//}
//# sourceMappingURL=num-helper.js.map